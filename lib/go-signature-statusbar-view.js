...
class GoSignatureStatusbarView {
  ...
  destroy() {
    ...
    if (this.configWidthSubscription) {
      this.configWidthSubscription.dispose()
    }
  ...
  subscribeToConfig() {
    ...
    this.configWidthSubscription = atom.config.observe('go-signature-statusbar.width', (value) => {
      if (value) {
        this.getElement().style.maxWidth = value
      } else {
        this.getElement().style.maxWidth = '20vw'
      }
    })
  }
